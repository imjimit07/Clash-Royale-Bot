...
def get_recordings_dir(app_name: str = "clbot", custom_path: str | None = None) -> str:
...
def get_recording_zips_dir(app_name: str = "clbot", custom_path: str | None = None) -> str:
...
def validate_recordings_path(custom_path: str | None) -> tuple[bool, str]:
    ...
    # A missing filesystem root/drive is never a valid custom recordings path.
    absolute = os.path.abspath(path)
    drive, _tail = os.path.splitdrive(absolute)
    if drive and not os.path.exists(drive + os.sep):
        return (False, "Folder does not exist and its drive is unavailable.")

    # On POSIX, do not bless an arbitrary first-level path directly under `/`.
    # A normal path such as `/tmp/clbot-recordings` has `/tmp` as an existing
    # ancestor and remains valid.
    ...
    while ancestor and not os.path.exists(ancestor):
        ...
        if parent == ancestor:  # reached a filesystem root that doesn't exist
            break
        ancestor = parent
    if (os.name != "nt" and ancestor == filesystem_root and absolute != filesystem_root) or not ancestor or not os.path.isdir(ancestor) or not os.access(ancestor, os.W_OK):
...
def recordings_total_bytes(app_name: str = "clbot", custom_path: str | None = None) -> int:
...
def recording_zips_total_bytes(app_name: str = "clbot", custom_path: str | None = None) -> int:
...
def recordings_total_bytes_all_locations(app_name: str = "clbot", custom_path: str | None = None) -> int:
...
def clear_recordings(app_name: str = "clbot", custom_path: str | None = None) -> tuple[int, int]:
