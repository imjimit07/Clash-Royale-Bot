...
from clbot.bot.coords import CHAMPION_ABILITY_DISMISS_COORD
...
def check_which_cards_are_available(emulator, check_side=False):
    ...
    for i, coords in enumerate(card_coords):
        ...
        # Avoid uint8 wrap-around during subtraction; 0-255 should be -255,
        # not 1, otherwise distant colours can pass the tolerance test.
        iar_int = iar_pixels.astype(numpy.int16)
        purple_int = purple_color.astype(numpy.int16)
        purple_pixels = numpy.all(numpy.abs(iar_int - purple_int) <= 30, axis=-1)
