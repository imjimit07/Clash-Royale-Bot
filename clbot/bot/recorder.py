...
royalestrat ``user_data`` importer):
    ...
    recordings/<slug>/
      ...
      plays.jsonl            # one JSON line per confirmed deploy plus a structured state snapshot
      manifest.json
...
DEFAULT_FPS = 3.0
PLAY_EVENT_SCHEMA = "decision-event/v1"
...
class FightPackRecorder:
    ...
    def start(
        ...
        self._plays_file = open(os.path.join(self.dir, "plays.jsonl"), "a", encoding="utf-8")
    ...
    def log_play(
        ...
        *,
        ...
        recent_cards: list[str] | None = None,
        policy_source: str = "unknown",
        confidence: float | None = None,
        placement_confidence: float | None = None,
        confirmed: bool = True,
    ) -> None:
        """Append a timestamped, self-describing action event.

        The state snapshot is intentionally stored with the action rather than
        reconstructed later from a 3 FPS video. This prevents frame/action timing
        drift and makes the resulting pack directly usable by the dataset builder.
        """
        if self.dir is None:
            return
        now = time.time()
        ...
        if confidence is not None:
            entry["confidence"] = round(float(confidence), 5)
        if placement_confidence is not None:
            entry["placement_confidence"] = round(float(placement_confidence), 5)
        state = {
            "elapsed_s": float(elapsed_s),
            "elixir": None if elixir is None else float(elixir),
            "hand": hand or [],
            "enemy_threat": threat or {"count": 0, "lane": "none", "x": 0.5, "y": 0.5},
            "target_lane": target_lane or "left",
            "recent_cards": recent_cards or [],
        }
        entry["state"] = state
        with self._lock:
            self._plays.append(entry)
            self._append_play_line_locked(entry)
    ...
    def log_decision(
        ...
        *,
        ...
        """Append a deliberate no-play decision for WAIT supervision."""
        if self.dir is None:
            return
        entry = {
            "action_type": "hold",
            "frame_index": self._frame_count,
            "timestamp_wall": round(time.time(), 6),
            "elapsed_s": round(float(elapsed_s), 3),
            "confirmed": True,
            "policy_source": str(policy_source),
            "reason": str(reason),
    ...
    def stop_capture(self) -> None:
        ...
        if self._writer is not None:
            ...
            self._writer = None
        with self._lock:
            if self._plays_file is not None:
                try:
                    self._plays_file.flush()
                    self._plays_file.close()
                except OSError:
                    pass
                self._plays_file = None
    ...
    def _append_play_line_locked(self, entry: dict) -> None:
        if self._plays_file is None:
            return
        try:
            self._plays_file.write(json.dumps(entry, separators=(",", ":")) + "\n")
            self._plays_file.flush()
    ...
    def _write_plays(self) -> None:
        assert self.dir is not None
        # Rewrite at finish to guarantee a compact, complete file after any partial
        # stream writes; the append stream above protects data against process loss.
    ...
    def _write_manifest(self, outcome: str | None) -> None:
        ...
        manifest = {
            ...
            "play_event_schema": PLAY_EVENT_SCHEMA,
        }
...
def log_play(
    ...
    **kwargs,
    ...
    """Record a play on the active recorder, including optional ML state metadata."""
    if _active is not None:
        _active.log_play(card_index, x, y, elapsed_s, **kwargs)


def log_decision(elapsed_s: float, **kwargs) -> None:
    """Record a no-play decision on the active recorder."""
    if _active is not None:
        _active.log_decision(elapsed_s, **kwargs)
